...
export const Niezbednik2Map: React.FC<Niezbednik2MapProps> = ({
  ...
  // ─── OSM markery dla Androida ─────────────────────────────────────────────

  const osmMarkers = useMemo<OSMMarker[]>(() => {
    return activePoints.map(p => {
      let color = TYPE_COLOR[p.type as FilterType] ?? '#EF4444';
      let icon: OSMMarker['icon'] = 'default';
      if (p.type === 'HOSPITAL') { icon = 'hospital'; }
      else if (p.type === 'PHARMACY') { color = p.is24h ? '#1E293B' : '#8B5CF6'; icon = 'pharmacy'; }
      else if (p.type === 'AED') { color = getAedColor(p.access, p.indoor); icon = 'aed'; }
      return {
        id: p.id,
        latitude: p.latitude,
  ...
  const handleOsmMarkerPress = useCallback((id: string) => {
